...
export class AmbientBackgroundComponent implements AfterViewInit, OnDestroy {
  ...
  private readonly XOMIFY_COLORS = [
    '#9c0abf', // xomify purple
    '#1bdc6f', // xomify green
    '#7b22d4', // purple variant
    '#14c95f', // green variant
  ];
  ...
  private placeStatic(): void {
    for (let i = 0; i < this.NUM_BLOBS; i++) {
      ...
      if (!body) continue;
      const color = this.XOMIFY_COLORS[i % this.XOMIFY_COLORS.length];
  ...
  private cycleColor(index: number): void {
    ...
    const startColorIndex = index % this.XOMIFY_COLORS.length;
    const colors = [
      ...this.XOMIFY_COLORS.slice(startColorIndex),
      ...this.XOMIFY_COLORS.slice(0, startColorIndex),
    ];
  ...
  private strikeLightning(): void {
    ...
    const color =
      Math.random() > 0.3
        ? this.XOMIFY_COLORS[0] // purple most of the time
        : this.XOMIFY_COLORS[
            Math.floor(Math.random() * this.XOMIFY_COLORS.length)
          ];
