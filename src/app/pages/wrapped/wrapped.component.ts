...
export class WrappedComponent implements OnInit {
  ...
  generatePlaylist(): void {
    if (!this.selectedWrap || this.displayTracks.length === 0) {
      this.toastService.showNegativeToast('No tracks available to generate a playlist');
      return;
    }
    ...
    const playlistName = `Xomify Wrapped — ${monthYear}`;
    const description = `My top ${this.displayTracks.length} tracks for ${monthYear}, generated by Xomify`;
    const trackUris = this.displayTracks.map(t => `spotify:track:${t.id}`);
    ...
    this.playlistService
      ...
      .subscribe({
        ...
        error: (err) => {
          console.error('Error generating wrapped playlist:', err);
          this.isGeneratingPlaylist = false;
